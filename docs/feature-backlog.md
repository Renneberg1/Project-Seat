...
- [x] LM Product Ideas Board Support (projects now carry `pi_project_key` alongside `pi_version`; dashboard `get_product_ideas()` branches JQL — PI board uses `versions[checkboxes]` + `Release Priority`/`PI State`, LM board uses the `Release` textfield + `Roadmap` field (`Now`/`Next`/`Later`/`Won't do`); `FIELD_RELEASE_TEXT` + `FIELD_ROADMAP` added to `jira_constants.py`; `JiraIssue.release_priority` falls back to Roadmap; `summarise_product_ideas` counts `Now` as must-have; cache key `pi:{key}:{version}` prevents PI/LM collisions; surfaced on spin-up, import, settings, and dashboard PI inline form)
